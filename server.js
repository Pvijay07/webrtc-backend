...
function getLocalIp() {
  ...
  for (let dev in ifaces) {
    const iface = ifaces[dev].filter(details => details.family === 'IPv4' && !details.internal);
...
async function startStream(camera_id, rtsp_url) {
  ...
  console.log(`PlainTransport created, listening on rtpPort: ${rtpPort}, rtcpPort: ${rtcpPort}`);
  ...
  ffmpegProcess.on('close', (code) => {
    console.log(`FFmpeg process for camera ${camera_id} exited with code ${code}`);
...
io.on('connection', async (socket) => {
  ...
  socket.on('consume', async ({ rtpCapabilities }, cb) => {
    try {
      if (!router.canConsume({ producerId: 'dummy', rtpCapabilities }) && false) {
